...
* (c) Media Soma GbR
*
* 0.1       06.06.2005  jn      1st release\n
* 0.11      14.07.2005  jn      fixed/added comments\n
* 0.2       26.09.2005  jn      changed to support more than one sort\n
* 0.3		09.02.2006	pr		merged ub_sort and ub_administration\n
* 								ub_sort became ub_administration\n
* 								lots of fixes/changes\n
* 0.31      28.03.2006  jn      fixed sort order\n
* 0.4		01.08.2006	pr		major rewrite, supports tree-style data
* 								code cleanup
* 0.41		31.08.2006	pr		added data-auto-patching for inconsistent
* 								array-lengths
*
*/

class ub_administration
{
    /**
    * class version
    */
    ...
    * 
    * @return       ub_administration (object)
    */
    public static function get_instance($ident, $label = null)
    {
        ...
        if (!isset($unibox->session->env->administration->$ident) || !isset(self::$instances[$ident]))
            self::$instances[$ident] = new ub_administration($ident, $label);
    ...
    */
    public function add_field($si_name, $width = 0, $sort_type = SORT_REGULAR, $sort = SORT_ASC)
    {
    	...
    	if (!isset($this->administration->sort->fields[$ident]))
    	{
	        ...
	        $this->administration->sort->fields[$ident]->index = count($this->administration->sort->fields) - 1;
            ...
            $this->administration->sort->fields[$ident]->sort_type = $sort_type;
	 ...
	 */
    public function sort_by($si_name)
    {
        if ($this->new)
        {
            $ident = substr(md5($this->ident.$si_name), 0, 10);
    ...
    */
	...
	protected function process_data($base)
	{
        ...
        foreach ($this->administration->sort->history as $index => $ident)
        {
        	// set current sort field
        	$sort = $this->administration->sort->fields[$ident];
        	
        	// loop through columns
			if (isset($columns[$sort->index]))
	        	foreach ($columns[$sort->index] as $dataset_index => $data)
	        	{
					// add static sort condition
					$args[0][$dataset_index] = (int)$base->children[$dataset_index]->static;
					if ($base->children[$dataset_index]->static)
						$args[2][$dataset_index] = $dataset_index;
					else
						$args[2][$dataset_index] = 0;
	
	        		// compute index offset
	        		$arg_index = 3 * $index + 4;
	        		if ($data->sort_value !== null)
	        			$args[$arg_index][$dataset_index] = $data->sort_value;
	        		else
	        			$args[$arg_index][$dataset_index] = strtolower($data->value);
	
					// add sort flags
					$args[$arg_index + 1] = $sort->sort;
	        		$args[$arg_index + 2] = $sort->sort_type;
	        	}
        }
        // workaround for PHP >= 5.2.0: sort keys instead of objects
        $keys = array_keys($base->children);
        $args[] = &$keys;
		...
		$children = array();
		foreach ($keys as $key)
			$children[] = $base->children[$key];
		$base->children = $children;
		
		// check if we're on root level
		if ($base === $this->root)
		{
			// if so, limit the amount of datasets that is displayed
	        if (!empty($base->children))
	        {
	            if (isset($this->unibox->session->env->pagebrowser->{$this->ident}))
	            {
	                $begin = $this->unibox->session->env->pagebrowser->{$this->ident}->begin;
	                $end = $this->unibox->session->env->pagebrowser->{$this->ident}->end;
	                if ($end - $begin >= $base->dataset_index)
	                    $end = $begin + $base->dataset_index - $base->static_count;
	            }
	            else
	            {
	                $begin = 0;
	                $end = $base->dataset_index - $base->static_count;
	            }
	        }
	        else
	            $begin = $end = 0;
		}
		// if not, show all datasets on the current level
		else
		{
            $begin = 0;
            $end = $base->dataset_index - $base->static_count;
		...
		// show all static datasets first
		for ($dataset_index = 0; $dataset_index < $base->static_count; $dataset_index++)
			$this->show_dataset($base->children[$dataset_index]);

        // loop through all ordinary datasets
        for ($dataset_index = $begin; $dataset_index < $end; $dataset_index++)
        	$this->show_dataset($base->children[$dataset_index + $base->static_count]);
	}
