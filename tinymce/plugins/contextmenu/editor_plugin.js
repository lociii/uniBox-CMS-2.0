...
// BEGIN ADD BY MEDIA SOMA
		elm = tinyMCE.getParentElement(elm, "img,table,td,acronym,a");
...
// BEGIN ADD BY MEDIA SOMA
				case "ACRONYM":
					contextMenu.addSeparator();
					contextMenu.addItem(tinyMCE.baseURL + "/themes/" + theme + "/images/acronym.gif", "$lang_acronym_desc", "mceAcronym");
					break;
				...
				case "IMG":
					...
					else
					{
						...
						curtheme = this.tinyMCE.getAttrib(elm, 'curtheme');

						if (curtheme != '__all')
						{
							for (var theme_ident in tinyMCE.settings.unibox_themes)
								if (curtheme == theme_ident)
									contextMenu.addItem(tinyMCE.baseURL + "/themes/" + theme + "/images/selected.gif", tinyMCE.settings.unibox_themes[theme_ident], "mceMediaSwitch", theme_ident, true);
								else						
									contextMenu.addItem('', tinyMCE.settings.unibox_themes[theme_ident], "mceMediaSwitch", theme_ident);
				
							contextMenu.addSeparator();
						}
						contextMenu.addItem(tinyMCE.baseURL + "/themes/" + theme + "/images/image.gif", "$lang_image_props_desc", "mceMedia");
					}
					break;
// END ADD BY MEDIA SOMA
