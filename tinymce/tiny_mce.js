...
// BEGIN ADD BY MEDIA SOMA
			if (tinyMCE.isMSIE && node.nodeValue.match(/version="1.0" encoding="UTF/))
				return '';
			else
...
// BEGIN ADD BY MEDIA SOMA
	// pass through replacement entity
	var decodedUrl = decodeURI(url);
	if (matches = decodedUrl.match(/\[replace [^\]]+\]/))
		return matches[0];
// END ADD BY MEDIA SOMA
...
// BEGIN ADD BY MEDIA SOMA
	// pass through replacement entity
	var decodedUrl = decodeURI(relative_url);
	if (matches = decodedUrl.match(/\[replace [^\]]+\]/))
		return matches[0];
// END ADD BY MEDIA SOMA
