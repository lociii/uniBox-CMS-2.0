...
<xsl:stylesheet version="1.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:php="http://php.net/xsl">
	<xsl:template match="content">
		...
		<script src="{/root/unibox/html_base}{$theme_base}/scripts/administration.js" type="text/javascript"></script>
        ...
        <xsl:for-each select="administration">
			<script type="text/javascript">
				<![CDATA[
					administrations.push(']]><xsl:value-of select="@ident" /><![CDATA[');
					args = new Array(']]><xsl:value-of select="$theme_base" /><![CDATA[');
					top.unibox_js.add_load_event('administrations_init', args);
					top.unibox_js.add_unload_event('administration_save_state');
				]]>
			</script>
                ...
                <form id="administration_{@ident}" action="administration_process" method="post">
					...
					<div class="table_administrate">
						...
						<table class="administrate">
							...
							<tbody id="administration_table_{@ident}">
	...
	<xsl:template match="dataset">
		...
		<xsl:variable name="theme_base" select="/root/unibox/theme_base[@template='shared_administration']/path"/>
		<script language="JavaScript">idents['<xsl:value-of select="$ident" />_<xsl:value-of select="$address" />'] = '<xsl:value-of select="$ident" />/<xsl:value-of select="@ident" />';</script>
		...
		<tr id="dataset_{$ident}_{$address}">
			...
			<xsl:choose>
				<xsl:when test="checkbox != ''">
					<td class="checkbox">
						<input type="checkbox" name="administration_checkbox[]" id="administration_checkbox_{$ident}_{$address}" value="{@ident}">
            ...
            <tr>
                ...
                <xsl:for-each select="column">
                    <th id="{../../@ident}_header_{position()}">
						...
						<xsl:if test="position() = 1 and normalize-space(../../datasets//dataset/dataset)">
							<xsl:attribute name="class">nesting</xsl:attribute>
						</xsl:if>
